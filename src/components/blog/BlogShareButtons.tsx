...
export default function BlogShareButtons({ url, title }: Props) {
  ...
  const shares = [
    ...
    },
    {
      ...
      icon: <LinkedInIcon />,
      className: 'bg-[#0A66C2] hover:bg-[#095fb3] text-white',
    },
  ];
    ...
    <div className="flex items-center gap-2">
      ...
      {shares.map((s) => (
        <a
          ...
          className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${s.className}`}
        >
          {s.icon}
        </a>
      ))}
      <button
        ...
        className="w-8 h-8 rounded-full flex items-center justify-center border border-gray-200 text-gray-500 hover:bg-gray-50 transition-colors"
      >
