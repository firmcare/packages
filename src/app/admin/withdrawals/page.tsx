...
export default async function WithdrawalsPage() {
  await requireAdmin();
  const withdrawals = await adminFetch<any[]>("/api/admin/withdrawals");

  const stats = {
    total: withdrawals.length,
    pending: withdrawals.filter(w => w.status === "PENDING").length,
    processing: withdrawals.filter(w => w.status === "PROCESSING").length,
    totalPaid: withdrawals.filter(w => w.status === "COMPLETED").reduce((s, w) => s + Number(w.amount), 0),
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Total Paid Out", value: "₦" + stats.totalPaid.toLocaleString("en-NG", { minimumFractionDigits: 2 }), color: "text-green-600" },
        ].map(s => (
